...
export default function BuildOutIcoArray({ ico, i, entry, pre }){
    // console.log('BuildOutIcoArray.  IcoArray is: ', IcoArray, ", entry is: ", entry); 
    const icoClass = ico.class ? ico.class : "inherit";
    const icoPre = ico.pre ? true : false;
    pre ? pre = true : pre = false;
    // console.log('line 9: ', icoPre, " vs ", pre);
    return (
        icoPre !== pre ?
        <></> :
